namespace Nancy.Validation.FluentValidation
{
    ...
    public class FluentValidationValidator : IModelValidator
    {
        ...
        private readonly FluentValidationValidatorFactory modelValidatorFactory;
        ...
        {
            var errors = ValidateSelfAndChildren(instance);

            return new ModelValidationResult(errors);
        }

        private IEnumerable<ModelValidationError> ValidateSelfAndChildren(object instance)
        {
            var rootResult = validator.Validate(instance);

            foreach (var error in GetErrors(rootResult))
            {
                yield return error;
            }
            ...
            {
                var child = property.GetValue(instance);

                if (null == child) continue;

                var childValidator = modelValidatorFactory.GetValidatorInstance(child.GetType());

                if (null == childValidator) continue;

                var childResult = childValidator.Validate(child);

                foreach (var error in GetErrors(childResult))
                {
                    yield return error;
                }
            }
